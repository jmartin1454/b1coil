# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 10:50:40 2026
...
Beff=sqrt(B1**2+((B0-(omega/gamma))**2))
sintheta=B1/Beff
costheta=(B0-(omega/gamma))/Beff
...
def spin_rot(t):
    sx=(sintheta*costheta)*(-np.cos(omegas*t)+1) 
    sy=sintheta*np.sin(omegas*t)
    sz=sintheta**2*np.cos(omegas*t)+costheta**2
    print("spin_rot",sx,sy,sz,np.sqrt(sx**2+sy**2+sz**2))
    return sx,sy,sz

def spin(t): # non-rotating frame
    sx=(sintheta)*(((-np.cos(omega*t))*(np.cos(omegas*t))*(costheta))+((np.cos(omega*t))*(costheta))+ ((np.sin(omega*t))*(np.sin(omegas*t))))
    sy=(sintheta)*(((np.sin(omega*t)*(np.cos(omegas*t)*(costheta))-((np.sin(omega*t))*(costheta))+((np.cos(omega*t))*(np.sin(omegas*t))))))
    sz=((np.cos(omegas*t))*(sintheta**2))+(costheta**2)
    print("spin",sx,sy,sz,np.sqrt(sx**2+sy**2+sz**2))
    return sx,sy,sz
...
print(t)
sx,sy,sz=spin(t)
sx_rot,sy_rot,sz_rot=spin_rot(t)
ax.set_xlim(-1,1)
ax.set_ylim(-1,1)
ax.set_zlim(-1,1)

line,=ax.plot(sx[:1],sy[:1],sz[:1])
line2,=ax.plot(sx_rot[:1],sy_rot[:1],sz_rot[:1])
a=Arrow3D([0,sx[0]],[0,sy[0]],[0,sz[0]],mutation_scale=20,arrowstyle="-|>",color="r")
a2=Arrow3D([0,sx_rot[0]],[0,sy_rot[0]],[0,sz_rot[0]],mutation_scale=20,arrowstyle="-|>",color="b")
ax.add_artist(a)
ax.add_artist(a2)
...
def update(num):
    ...
    line2.set_xdata(sx_rot[:num])
    line2.set_ydata(sy_rot[:num])
    line2.set_3d_properties(sz_rot[:num])
    a._verts3d=[0,sx[num]],[0,sy[num]],[0,sz[num]]
    a2._verts3d=[0,sx_rot[num]],[0,sy_rot[num]],[0,sz_rot[num]]
